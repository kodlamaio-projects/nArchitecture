...
public class BrandBusinessRules
{
    ...
    public async Task BrandIdShouldExistWhenSelected(int id)
    {
        ...
        if (result == null) throw new BusinessException(BrandExceptionMessages.BrandNotExistsMessage);
    }

    public async Task BrandNameCanNotBeDuplicatedWhenInserted(string name)
    {
        IPaginate<Brand> result = await _brandRepository.GetListAsync(b => b.Name == name);
        if (result.Items.Any()) throw new BusinessException(BrandExceptionMessages.BrandNameExistsMessage);
    }
}